...
async function getMeteoDataSjenica(dateToStart: string, daysToCheck: number) {
    ...
    let cityName: string = "Sjenica"
    //Work forward from start date


    for (let i: number = 0; i < daysToCheck; ++i) {
        //Add axios promise to array
        promiseArray.push(meteo.getCityMaxWeather(date.format("YYYY-MM-DD"), 43.2722, 20.0060));
...
getMeteoDataSjenica('2021-04-15', 100);